...
/**
 * @summary Object that holds the command line options.
 * @author Ricardo Sousa - Challenge.IT
 */
var commandLineOptions = JSON.parse(fs.readFileSync(__dirname + '/commandLineOptions.json', 'utf8'));
...
function doSoapRequest(url, method, soapAction, data, inputModel, outputModel, ns, callback) {
  ...
  // add the private key, certificate and CA certs of the client in PFX or PKCS12 format.
  if(commandLineOptions.pfx && commandLineOptions.passphrase)
  {
    soapRequest.strictSSL = true;
    soapRequest.pfx = fs.readFileSync(commandLineOptions.pfx);
    soapRequest.passphrase = commandLineOptions.passphrase;
    soapRequest.agent = new https.Agent(soapRequest);
  }
  
  // TODO: Add support for PEM format.
